...
unsigned int SimpleAudioEngineOpenSL::playEffect(const char* pszFilePath, bool bLoop, const char* szXPFPath, unsigned int dwSeek, unsigned int dwSize)
{
	unsigned int soundID;
	do 
	{
		soundID = s_pOpenSL->preloadEffect(pszFilePath, szXPFPath, dwSeek, dwSize);
		if (soundID != FILE_NOT_FOUND)
		{
			if (s_pOpenSL->getEffectState(soundID) == PLAYSTATE_PLAYING)
			{
