...
bool initAudioPlayer(AudioPlayer* player, const char* filename, unsigned int dwSeek, unsigned int dwSize) 
{
	static FILE* fp = NULL;
	if(fp)
		fclose(fp);
	...
	if(fp){
		...
		SLDataLocator_AndroidFD loc_fd = {SL_DATALOCATOR_ANDROIDFD, fd, dwSeek, dwSize};
		SLDataFormat_MIME format_mime = {SL_DATAFORMAT_MIME, NULL, SL_CONTAINERTYPE_UNSPECIFIED};
		player->audioSrc.pLocator = &loc_fd;
		player->audioSrc.pFormat = &format_mime;
		bool bRes = createAudioPlayerBySource(player);
		if(!bRes)
			LOGD("[initAudioPlayer] createAudioPlayerBySource failed : %s at %d len %d", filename, dwSeek, dwSize);
		return bRes;
	}
	LOGD("[initAudioPlayer] file not found! Stop preload file: %s at %d len %d", filename, dwSeek, dwSize);
	return false;
}
...
unsigned int OpenSLEngine::preloadEffect(const char * filename, const char* szXPFPath, unsigned int dwSeek, unsigned int dwSize)
{
	unsigned int nID = _Hash(filename);
	// if already exists
	EffectList::iterator p = sharedList().find(nID);
	...
	AudioPlayer* player = new AudioPlayer();
	if (!initAudioPlayer(player, szXPFPath, dwSeek, dwSize))
	{
		free(player);
		return FILE_NOT_FOUND;
	}
